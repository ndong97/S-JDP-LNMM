...
class Gibbs_Survival_DP_subset:
        ...
        def fit(self, X, y, censoring, subset_name):
            self.subset_name = subset_name
            self.predictor = X
            self.n, self.p = self.predictor.shape
            self.predictor_name = []
            for i in range(self.p):
                self.predictor_name.append(f'X{i+1}')
            self.predictor.columns = self.predictor_name
            self.response = y
            self.response_name = 'y'
            self.response.name=self.response_name
            self.censoring = censoring
            self.censoring_name = 'event'
            self.censoring.name= self.censoring_name
            self.data = pd.concat([self.predictor, self.response, self.censoring], axis=1)
            self.data_records = self.data.copy()
            self.initialize()
            ...
            for j in range(1, self.burnin+self.B*self.thinning+1):
                ...
                for i in range(self.n):
                    ...
                    new_beta_tau = self.tau_y[int(z_y)]*inv(c_Y)
                    ...
                    # print(new_tau_scale)
                    new_tau = stats.invgamma.rvs(a=new_tau_shape, scale=new_tau_scale, size=100)
                    ...
                    if z_y == (n_N.shape[0]-1):
                        ...
                        self.tau_y = np.append(self.tau_y, new_tau[z_beta_y])
                ...
                self.data['z_y'] = self.data['z_y'].replace(unorder_z_y, np.arange(self.beta_y.shape[0]))

                unorder_z_x = np.sort(self.data['z_x'].unique())
                self.beta_x = self.beta_x[unorder_z_x]
                self.tau_x = self.tau_x[unorder_z_x]
                self.data['z_x'] = self.data['z_x'].replace(unorder_z_x, np.arange(self.beta_x.shape[0]))
                ...
                for i in range(self.beta_y.shape[0]):
                    ...
                    mean_y_uncensored = matmul(X_uncensored, self.beta_y.T)
                    mean_y_censored = matmul(X_censored, self.beta_y.T)
                    pdf = np.log(vectorized_norm_logpdf(X=y_uncensored, means=mean_y_uncensored, covariances=self.tau_y))[i]
                    sf = np.log(vectorized_norm_logsf(X=y_censored, means=mean_y_censored, covariances=self.tau_y))[i]
                    likelihood_iter.append(pdf+sf)
                loglikelihood_y_recording.append(sum(likelihood_iter))
                log_z_y_trace.append(np.log(self.data['z_y'].sum()))
                n_cluster_recording.append(self.beta_y.shape[0])
                tau_y_mean_recording.append(self.tau_y.mean())
                ...
                if (j >= self.burnin - 1 + self.thinning) & ((j + 1 - self.burnin) % self.thinning == 0):
                    ...
                    z_y_recording.append(self.data['z_y'].copy())
                    z_x_recording.append(self.data['z_x'].copy())
            ...
            self.posterior_samples = {'mu_theta': mu_theta_recording, 'mu_psi': mu_psi_recording, 'z_y': z_y_recording, 
                                                                      ...
                                                                      'log_z_y_trace': log_z_y_trace, 'tau_y_mean_recording': tau_y_mean_recording, 'n_cluster_recording': n_cluster_recording}
            end_0 = time.time()
            print(end_0-start_0)
        
        def posterior_samples_(self):
            samples = self.posterior_samples
            records = pd.DataFrame({'mu_theta': samples['mu_theta'], 'mu_psi': samples['mu_psi'], 'loglike_y_recording': samples['loglikelihood_y'], 'log_z_y_trace': samples['log_z_y_trace'], 'tau_y_mean_recording': samples['tau_y_mean_recording'], 'n_cluster_recording':samples['n_cluster_recording']})
            records.to_csv(f'{self.subset_name}_records_for_figures.csv')
            z_y_name = []
            z_x_name = []
            for i in range(self.B):
                z_y_name.append(f'z_y_{i+1}_iteration')
                z_x_name.append(f'z_x_{i+1}_iteration')
            Z_Y = pd.DataFrame(np.array(samples['z_y']).T, columns=z_y_name)
            Z_X = pd.DataFrame(np.array(samples['z_x']).T, columns=z_x_name)
            Z_Y.to_csv(f'{self.subset_name}_Z_Y_records.csv')
            Z_X.to_csv(f'{self.subset_name}_Z_X_records.csv')
            beta_y_name = []
            tau_y_name = []
            for i in range(self.B):
                beta_y__name = []
                tau_y__name = []
                for j in range(len(samples['beta_y'][i])):
            ...
            def flatten_extend(matrix):
                ...
                return flat_list
            flatten_beta_y = flatten_extend(samples['beta_y'])
            flatten_beta_y_name = flatten_extend(beta_y_name)
            BETA_Y = pd.DataFrame(np.array(flatten_beta_y).T, columns=flatten_beta_y_name)
            BETA_Y.to_csv(f'{self.subset_name}_BETA_Y_records.csv')
            flatten_tau_y = flatten_extend(samples['tau_y'])
            ...
            TAU_Y.to_csv(f'{self.subset_name}_TAU_Y_records.csv')
            beta_x_name = []
            tau_x_name = []
            for i in range(self.B):
                beta_x__name = []
                tau_x__name = []
                for j in range(len(samples['beta_x'][i])):
            ...
            flatten_beta_x = flatten_extend(samples['beta_x'])
            flatten_beta_x_name = flatten_extend(beta_x_name)
            BETA_X = pd.DataFrame(np.array(flatten_beta_x).T, columns=flatten_beta_x_name)
            BETA_X.to_csv(f'{self.subset_name}_BETA_X_records.csv')
            flatten_tau_x = flatten_extend(samples['tau_x'])
            flatten_tau_x_name = flatten_extend(tau_x_name)
            TAU_X = pd.DataFrame(np.array(flatten_tau_x).T, columns=flatten_tau_x_name)
            TAU_X.to_csv(f'{self.subset_name}_TAU_X_records.csv')



np.random.seed(555)
random.seed(555)
dat = pd.read_csv('''G:/OneDrive - Texas Tech University/Documents/Scalable Survival-DP Package/Pkg for scalable survival DP/3-clusters/Simulation_3_cluster_subset_1.csv''', index_col=0)
y = np.log(dat['y'])
censoring = dat['C']
X = dat.drop(columns=['Z', 'C', 'y'])
EDP = Gibbs_Survival_DP_subset(N=5, M=5, burnin=50, thinning=2, B=5, random_state=777)
EDP.fit(X=X, y=y, censoring=censoring, subset_name='Simulation 3_1')
EDP.posterior_samples_()